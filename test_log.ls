...
switch
  | process.argv.2 is 'client'
    pipe = fork './.build/logpipe.js', [process.argv.2, tcp_port], process.argv.2, ->
    ...
    trans = ->
      ...
      switch state.i++
        | 0
          state.c1 = fork './.build/test_log.js', ['client'], 'c1', trans
        ...
        | 2
          state.c1.send 'exit'
        | 3
          state.c2 = fork './.build/test_log.js', ['client'], 'c2', trans
          setTimeout trans, 200
        | 4
          state.s1 = fork './.build/logserver.js', [tcp_port, http_port], 's1', trans
          setTimeout trans, 200
        | 5
          state.c2.send 'm2'
        ...
        | 7
          state.s2 = fork './.build/logserver.js', [tcp_port, http_port], 's2', trans
          setTimeout trans, 200
        | 8
          state.c2.send 'm3'
          setTimeout trans, connect_timeout+200
        | 9
          state.c2.send 'exit'
        | 10
          state.c3 = fork './.build/test_log.js', ['client'], 'c3', trans
          setTimeout trans, 200
        | 11
          state.c3.send 'm4'
          setTimeout trans, 200
