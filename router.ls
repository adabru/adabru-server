...
start_router = (hostname, port, cb) ->
  ...
  https.createServer signing, (req, res) ->
    ...
    options = {hostname: '::1', port: routes[r], path: req.url, req.headers, req.method}
...
heartbeat = ->
  ...
  if not state.router?
    state.mtime = mtime
    state.router = start_router hostname, port, -> console.log "router running at https://#{hostname}:#{port}/"
  else if (state.mtime.key isnt mtime.key) or (state.mtime.cert isnt mtime.cert)
    console.log 'reloading SSL certificate'
    state.mtime = mtime
    state.router.close -> state.router = start_router hostname, port, ->

# 80 → 443 redirect
http.createServer (req, res) ->
  res.writeHead 302, 'location': "https://#{/[^:]*/.exec(req.headers['host']).0}:#{http_redirect.to}#{req.url}"
  res.end!
.listen http_redirect.from, hostname, -> console.log "http redirecting from http://#{hostname}:#{http_redirect.from}"
